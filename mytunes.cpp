...
void MyTunes::executeCMDFOLLOW(Command cmd) {
	...
	enum arguments {FOLLOW, _U, USERID, _P,PLAYLIST_NAME,_F,USERID2};

	// command of stop follow a playlist
	if(cmd.isTokenAt(USERID2, "stop")){
        User * user = users.findByID(cmd.getToken(USERID));
        Playlist * followPlaylist = user->findPlaylist(cmd.getToken(PLAYLIST_NAME));
        followPlaylist->stopFollowing();

    }
		// command of follow a playlist
	else if (cmd.hasToken("-u")&&cmd.hasToken("-f")){
		User * user = users.findByID(cmd.getToken(USERID));
		User * user2 = users.findByID(cmd.getToken(USERID2));
        Playlist * playlist = user2->findPlaylist(cmd.getToken(PLAYLIST_NAME));
		if(user == NULL || user2 == NULL) {
            view.printOutput(" user does not exsits\n");
            return;
        }
		// if user2's playlist does not exist
		else if (playlist == NULL){
			view.printOutput(cmd.getToken(PLAYLIST_NAME) + " dose not exist\n");
            return;
		}

			// start follow user2's playlist
		else {
            Playlist *copyPlaylist = new Playlist(cmd.getToken(PLAYLIST_NAME));
            Playlist * followPlaylist = user->findPlaylist(cmd.getToken(PLAYLIST_NAME));
            copyPlaylist->startFollowing(*playlist);

            if(followPlaylist != NULL){
                followPlaylist->startFollowing(*playlist);

            } else{
                user->addPlaylist(*copyPlaylist );
            }


        }
	}
	else view.printOutput("?????????");
